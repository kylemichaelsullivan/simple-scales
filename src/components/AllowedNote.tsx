...
function AllowedNote({ note, isTonic }: AllowedNoteProps) {
  ...
  return (
    <span
      className={`AllowedNote absolute flex items-center justify-center ${bgColor} rounded-full text-white ${fontSize} font-bold text-center leading-none w-4 h-4 bottom-1 left-1/2 translate-x-[-50%] ${hasFlat ? 'hasFlat' : ''} ${hasSharp ? 'hasSharp' : ''}`}
      title={note}
    >
      {showNoteLabels ? note : ''}
    </span>
